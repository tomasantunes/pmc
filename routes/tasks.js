...
router.get("/api/get-tasks-from-folder", (req, res) => {
  ...
  con.query(sql, [folder_id], function (err, result) {
    ...
    for (var i in result) {
      if (
        result[i].time == "1970-01-01 01:00:00 - 1970-01-01 01:00:00" ||
        result[i].time == "1970-01-01 00:00:00 - 1970-01-01 00:00:00"
      ) {
        result[i].time = "";
      }
    }
...
router.post("/api/edit-task", (req, res) => {
  ...
  if (
    ...
    start_time = new Date("1970-01-01Z00:00:00:000")
      .toISOString()
      .slice(0, 19)
      .replace("T", " ");
    end_time = new Date("1970-01-01Z00:00:00:000")
      .toISOString()
      .slice(0, 19)
      .replace("T", " ");
  }
